...
{
    public class VoiceoverHistoryView
    {
        ...
        public VoiceoverHistoryView(VoiceoverAudioPlayer player)
        {
        ...
        public async void FetchHistory()
        {
            ...
            if (history != null)
            {
                ...
                // I'll add a Repaint callback/event if needed, but for now `VoiceoverAudioPlayer` has a reference to Window, 
        ...
        public void DrawList()
        {
            ...
            foreach (var item in apiHistory)
            {
                ...
                GUIStyle rowStyle = isSelected ? VoiceoverEditorStyles.TabBtnActiveStyle : EditorStyles.label;
        ...
        private async void LoadHistoryItem(HistoryItemApiModel item)
        {
             ...
             var clip = await VoiceoverAPI.GetHistoryAudioAsync(item.history_item_id);
             if (clip != null)
             {
                 ...
                 {
                     Debug.LogError($"[Voiceover] Failed to load audio clip for {item.history_item_id}");
                 }
             }
        }
        ...
        private async void DownloadHistoryAudio(HistoryItemApiModel item)
        {
            ...
            var audioData = await VoiceoverAPI.GetHistoryAudioBytesAsync(item.history_item_id);
            if (audioData != null)
            {
                try
                {
                ...
                {
                    Debug.LogError($"[Voiceover] Save failed: {e.Message}");
